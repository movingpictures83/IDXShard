...
#class IDXShardPlugin:
#   def input(args, inputfile):
#        args.parameters = PyIO.readParameters(inputfile)
#        args.bowtie2_index = PyPluMA.prefix()+"/"+args.parameters["bowtie2_index"]
#        args.shards = int(args.parameters["shards"])
#   def run(args):
#        pass
#   def output(args, outputfile):
def main(args):
    ...
    if args.verbose:
        ...
        for item, count in colections.Counter(acknowledge_list).items():
            if (count > 1):
                print(item)
